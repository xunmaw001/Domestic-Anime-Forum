<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.dongmanfenlei" type="text" placeholder="动漫分类" ></input>
			</view>
                  			<view v-if="queryIndex==2" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.biaoqian" type="text" placeholder="标签" ></input>
			</view>
        ...
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item" :style='{"padding":"24rpx 0 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 20rpx 0","borderColor":"red","backgroundColor":"rgba(98, 188, 203, 1)","borderRadius":"8rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
            ...
            <text v-if="userid && isAuth('dongmanwenzhang','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('dongmanwenzhang','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('dongmanwenzhang','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('dongmanwenzhang','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.dongmanfenlei="";
				this.searchForm.biaoqian="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.dongmanfenlei){
					params['dongmanfenlei'] = '%' + this.searchForm.dongmanfenlei + '%'
				}
				if(this.searchForm.biaoqian){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`dongmanwenzhang`, params);
                } else {
                    res = await this.$api.list(`dongmanwenzhang`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('dongmanwenzhang', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.dongmanfenlei){
					searchForm['dongmanfenlei'] = '%' + this.searchForm.dongmanfenlei + '%'
				}
				if(this.searchForm.biaoqian){
					searchForm['biaoqian'] = '%' + this.searchForm.biaoqian + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`dongmanwenzhang`, searchForm);
                } else {
                    res = await this.$api.list(`dongmanwenzhang`, searchForm);
                }
				// 如果是第一页数据置空
